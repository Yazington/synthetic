...
__import__('.'.join(__package__.split('.')[:-1] + ['PIL']), globals(), locals(), [], 0)
from .. import PIL
from . import Image
from ._deprecate import deprecate
modules = {'pil': ('PIL._imaging', 'PILLOW_VERSION'), 'tkinter': ('PIL._tkinter_finder', 'tk_version'), 'freetype2': ('PIL._imagingft', 'freetype2_version'), 'littlecms2': ('PIL._imagingcms', 'littlecms_version'), 'webp': ('PIL._webp', 'webpdecoder_version')}
...
codecs = {'jpg': ('jpeg', 'jpeglib'), 'jpg_2000': ('jpeg2k', 'jp2klib'), 'zlib': ('zip', 'zlib'), 'libtiff': ('libtiff', 'libtiff')}
...
features: dict[str, tuple[str, str | bool, str | None]] = {'webp_anim': ('PIL._webp', True, None), 'webp_mux': ('PIL._webp', True, None), 'transp_webp': ('PIL._webp', True, None), 'raqm': ('PIL._imagingft', 'HAVE_RAQM', 'raqm_version'), 'fribidi': ('PIL._imagingft', 'HAVE_FRIBIDI', 'fribidi_version'), 'harfbuzz': ('PIL._imagingft', 'HAVE_HARFBUZZ', 'harfbuzz_version'), 'libjpeg_turbo': ('PIL._imaging', 'HAVE_LIBJPEGTURBO', 'libjpeg_turbo_version'), 'libimagequant': ('PIL._imaging', 'HAVE_LIBIMAGEQUANT', 'imagequant_version'), 'xcb': ('PIL._imaging', 'HAVE_XCB', None)}
...
def pilinfo(out: IO[str] | None=None, supported_formats: bool=True) -> None:
    ...
    print('-' * 68, file=out)
    print(f'Python Pillow modules loaded from {os.path.dirname(Image.__file__)}', file=out)
    print(f'Binary Pillow modules loaded from {os.path.dirname(Image.core.__file__)}', file=out)
    print('-' * 68, file=out)
    for (name, feature) in [('pil', 'PIL CORE'), ('tkinter', 'TKINTER'), ('freetype2', 'FREETYPE2'), ('littlecms2', 'LITTLECMS2'), ('webp', 'WEBP'), ('jpg', 'JPEG'), ('jpg_2000', 'OPENJPEG (JPEG2000)'), ('zlib', 'ZLIB (PNG/ZIP)'), ('libtiff', 'LIBTIFF'), ('raqm', 'RAQM (Bidirectional Text)'), ('libimagequant', 'LIBIMAGEQUANT (Quantization method)'), ('xcb', 'XCB (X protocol)')]:
        if check(name):
            v: str | None = None
            if name == 'jpg':
