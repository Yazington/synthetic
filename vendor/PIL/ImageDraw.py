...
class ImageDraw:
    ...
    def chord(self, xy: Coords, start: float, end: float, fill: _Ink | None=None, outline: _Ink | None=None, width: int=1) -> None:
        """Draw a chord."""
        ...
        if ink is not None:
            ...
            if joint == 'curve' and width > 4:
                ...
                for i in range(1, len(points) - 1):
                    point = points[i]
                    angles = [math.degrees(math.atan2(end[0] - start[0], start[1] - end[1])) % 360 for (start, end) in ((points[i - 1], point), (point, points[i + 1]))]
    ...
    def pieslice(self, xy: Coords, start: float, end: float, fill: _Ink | None=None, outline: _Ink | None=None, width: int=1) -> None:
        """Draw a pieslice."""
        (ink, fill_ink) = self._getink(outline, fill)
        if fill_ink is not None:
    ...
    def regular_polygon(self, bounding_circle: Sequence[Sequence[float] | float], n_sides: int, rotation: float=0, fill: _Ink | None=None, outline: _Ink | None=None, width: int=1) -> None:
    ...
    def rounded_rectangle(self, xy: Coords, radius: float=0, fill: _Ink | None=None, outline: _Ink | None=None, width: int=1, *, corners: tuple[bool, bool, bool, bool] | None=None) -> None:
        ...
        def draw_corners(pieslice: bool) -> None:
            ...
            if full_x:
                ...
                parts = tuple((part for (i, part) in enumerate((((x0, y0, x0 + d, y0 + d), 180, 270), ((x1 - d, y0, x1, y0 + d), 270, 360), ((x1 - d, y1 - d, x1, y1), 0, 90), ((x0, y1 - d, x0 + d, y1), 90, 180))) if corners[i]))
            for part in parts:
                if pieslice:
    ...
    def _multiline_spacing(self, font: ImageFont.ImageFont | ImageFont.FreeTypeFont | ImageFont.TransposedFont, spacing: float, stroke_width: float) -> float:
    ...
    def text(self, xy: tuple[float, float], text: AnyStr, fill: _Ink | None=None, font: ImageFont.ImageFont | ImageFont.FreeTypeFont | ImageFont.TransposedFont | None=None, anchor: str | None=None, spacing: float=4, align: str='left', direction: str | None=None, features: list[str] | None=None, language: str | None=None, stroke_width: float=0, stroke_fill: _Ink | None=None, embedded_color: bool=False, *args: Any, **kwargs: Any) -> None:
        ...
        def draw_text(ink: int, stroke_width: float=0) -> None:
            ...
            try:
                (mask, offset) = font.getmask2(text, mode, *args, direction=direction, features=features, language=language, stroke_width=stroke_width, anchor=anchor, ink=ink, start=start, **kwargs) # type: ignore[union-attr,misc]
                coord = [coord[0] + offset[0], coord[1] + offset[1]]
            except AttributeError:
                try:
                    mask = font.getmask(text, mode, direction, features, language, stroke_width, anchor, ink, *args, start=start, **kwargs) # type: ignore[misc]
                except TypeError:
    ...
    def textlength(self, text: AnyStr, font: ImageFont.ImageFont | ImageFont.FreeTypeFont | ImageFont.TransposedFont | None=None, direction: str | None=None, features: list[str] | None=None, language: str | None=None, embedded_color: bool=False, *, font_size: float | None=None) -> float:
...
def getdraw(im: Image.Image | None=None, hints: list[str] | None=None) -> tuple[ImageDraw2.Draw | None, ModuleType]:
    """
    :param im: The image to draw in.
...
def floodfill(image: Image.Image, xy: tuple[int, int], value: float | tuple[int, ...], border: float | tuple[int, ...] | None=None, thresh: float=0) -> None:
    """
...
def _compute_regular_polygon_vertices(bounding_circle: Sequence[Sequence[float] | float], n_sides: int, rotation: float) -> list[tuple[float, float]]:
    """
    ...
    if len(bounding_circle) == 3:
        ...
        if not all((isinstance(i, (int, float)) for i in bounding_circle[0])) or not isinstance(bounding_circle[1], (int, float)):
            msg = 'bounding_circle should only contain numeric data'
            raise ValueError(msg)
