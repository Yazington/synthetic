...
MODES = ['1', 'CMYK', 'F', 'HSV', 'I', 'I;16', 'I;16B', 'I;16L', 'I;16N', 'L', 'LA', 'La', 'LAB', 'P', 'PA', 'RGB', 'RGBA', 'RGBa', 'RGBX', 'YCbCr']
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self.mode == other.mode and self.size == other.size and (self.info == other.info) and (self.getpalette() == other.getpalette()) and (self.tobytes() == other.tobytes())
    ...
    def convert(self, mode: str | None=None, matrix: tuple[float, ...] | None=None, dither: Dither | None=None, palette: Palette=Palette.WEB, colors: int=256) -> Image:
        ...
        if mode == 'P' and palette == Palette.ADAPTIVE:
            ...
            if trns is not None:
                try:
                    new_im.info['transparency'] = new_im.palette.getcolor(cast(tuple[int, ...], trns), new_im)
                except Exception:
                    del new_im.info['transparency']
    ...
    def paste(self, im: Image | str | float | tuple[float, ...], box: Image | tuple[int, int, int, int] | tuple[int, int] | None=None, mask: Image | None=None) -> None:
        """
    ...
    def point(self, lut: Sequence[float] | NumpyArray | Callable[[int], float] | Callable[[ImagePointTransform], ImagePointTransform | float] | ImagePointHandler, mode: str | None=None) -> Image:
        """
    ...
    def putdata(self, data: Sequence[float] | Sequence[Sequence[int]] | core.ImagingCore | NumpyArray, scale: float=1.0, offset: float=0.0) -> None:
        """
    ...
    def _get_safe_box(self, size: tuple[int, int], resample: Resampling, box: tuple[float, float, float, float]) -> tuple[int, int, int, int]:
    ...
    def resize(self, size: tuple[int, int] | list[int] | NumpyArray, resample: int | None=None, box: tuple[float, float, float, float] | None=None, reducing_gap: float | None=None) -> Image:
        """
    ...
    def rotate(self, angle: float, resample: Resampling=Resampling.NEAREST, expand: int | bool=False, center: tuple[float, float] | None=None, translate: tuple[int, int] | None=None, fillcolor: float | tuple[float, ...] | str | None=None) -> Image:
        """
    ...
    def thumbnail(self, size: tuple[float, float], resample: Resampling=Resampling.BICUBIC, reducing_gap: float | None=2.0) -> None:
        """
    ...
    def transform(self, size: tuple[int, int], method: Transform | ImageTransformHandler | SupportsGetData, data: Sequence[Any] | None=None, resample: int=Resampling.NEAREST, fill: int=1, fillcolor: float | tuple[float, ...] | str | None=None) -> Image:
        """
    ...
    def __transformer(self, box: tuple[int, int, int, int], image: Image, method: Transform, data: Sequence[float], resample: int=Resampling.NEAREST, fill: bool=True) -> None:
        w = box[2] - box[0]
        h = box[3] - box[1]
        ...
        if resample not in (Resampling.NEAREST, Resampling.BILINEAR, Resampling.BICUBIC):
            if resample in (Resampling.BOX, Resampling.HAMMING, Resampling.LANCZOS):
                unusable: dict[int, str] = {Resampling.BOX: 'Image.Resampling.BOX', Resampling.HAMMING: 'Image.Resampling.HAMMING', Resampling.LANCZOS: 'Image.Resampling.LANCZOS'}
...
def frombytes(mode: str, size: tuple[int, int], data: bytes | bytearray | SupportsArrayInterface, decoder_name: str='raw', *args: Any) -> Image:
    """
...
def open(fp: StrOrBytesPath | IO[bytes], mode: Literal['r']='r', formats: list[str] | tuple[str, ...] | None=None) -> ImageFile.ImageFile:
    """
    ...
    if mode != 'r':
        ...
        msg = 'StringIO cannot be used to open an image. Binary data must be used instead.' # type: ignore[unreachable]
        raise ValueError(msg)
    if formats is None:
        formats = ID
    ...
    def _open_core(fp: IO[bytes], filename: str | bytes, prefix: bytes, formats: list[str] | tuple[str, ...]) -> ImageFile.ImageFile | None:
        for i in formats:
            i = i.upper()
            if i not in OPEN:
                init()
...
def register_open(id: str, factory: Callable[[IO[bytes], str | bytes], ImageFile.ImageFile] | type[ImageFile.ImageFile], accept: Callable[[bytes], bool | str] | None=None) -> None:
    """
