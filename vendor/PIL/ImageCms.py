...
class ImageCmsTransform(Image.ImagePointHandler):
    ...
    def __init__(self, input: ImageCmsProfile, output: ImageCmsProfile, input_mode: str, output_mode: str, intent: Intent=Intent.PERCEPTUAL, proof: ImageCmsProfile | None=None, proof_intent: Intent=Intent.ABSOLUTE_COLORIMETRIC, flags: Flags=Flags.NONE):
        supported_modes = ('RGB', 'RGBA', 'RGBX', 'CMYK', 'I;16', 'I;16L', 'I;16B', 'YCbCr', 'LAB', 'L', '1')
        for mode in (input_mode, output_mode):
            if mode not in supported_modes:
                deprecate(mode, 12, {'L;16': 'I;16 or I;16L', 'L:16B': 'I;16B', 'YCCA': 'YCbCr', 'YCC': 'YCbCr'}.get(mode))
        if proof is None:
...
def profileToProfile(im: Image.Image, inputProfile: _CmsProfileCompatible, outputProfile: _CmsProfileCompatible, renderingIntent: Intent=Intent.PERCEPTUAL, outputMode: str | None=None, inPlace: bool=False, flags: Flags=Flags.NONE) -> Image.Image | None:
    """
...
def getOpenProfile(profileFilename: str | SupportsRead[bytes] | core.CmsProfile) -> ImageCmsProfile:
    """
...
def buildTransform(inputProfile: _CmsProfileCompatible, outputProfile: _CmsProfileCompatible, inMode: str, outMode: str, renderingIntent: Intent=Intent.PERCEPTUAL, flags: Flags=Flags.NONE) -> ImageCmsTransform:
    """
...
def buildProofTransform(inputProfile: _CmsProfileCompatible, outputProfile: _CmsProfileCompatible, proofProfile: _CmsProfileCompatible, inMode: str, outMode: str, renderingIntent: Intent=Intent.PERCEPTUAL, proofRenderingIntent: Intent=Intent.ABSOLUTE_COLORIMETRIC, flags: Flags=Flags.SOFTPROOFING) -> ImageCmsTransform:
    """
    (pyCMS) Builds an ICC transform mapping from the ``inputProfile`` to the
...
def versions() -> tuple[str, str | None, str, str]:
    ...
    """
    deprecate('PIL.ImageCms.versions()', 12, '(PIL.features.version("littlecms2"), sys.version, PIL.__version__)')
