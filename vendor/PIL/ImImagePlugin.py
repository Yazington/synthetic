...
OPEN = {'0 1 image': ('1', '1'), 'L 1 image': ('1', '1'), 'Greyscale image': ('L', 'L'), 'Grayscale image': ('L', 'L'), 'RGB image': ('RGB', 'RGB;L'), 'RLB image': ('RGB', 'RLB'), 'RYB image': ('RGB', 'RLB'), 'B1 image': ('1', '1'), 'B2 image': ('P', 'P;2'), 'B4 image': ('P', 'P;4'), 'X 24 image': ('RGB', 'RGB'), 'L 32 S image': ('I', 'I;32'), 'L 32 F image': ('F', 'F;32'), 'RGB3 image': ('RGB', 'RGB;T'), 'RYB3 image': ('RGB', 'RYB;T'), 'LA image': ('LA', 'LA;L'), 'PA image': ('LA', 'PA;L'), 'RGBA image': ('RGBA', 'RGBA;L'), 'RGBX image': ('RGB', 'RGBX;L'), 'CMYK image': ('CMYK', 'CMYK;L'), 'YCC image': ('YCbCr', 'YCbCr;L')}
        ...
        if self.rawmode[:2] == 'F;':
            try:
                ...
                if bits not in [8, 16, 32]:
                    self.tile = [ImageFile._Tile('bit', (0, 0) + self.size, offs, (bits, 8, 3, 0, -1))]
                    return
            except ValueError:
                pass
        if self.rawmode in ['RGB;T', 'RYB;T']:
            size = self.size[0] * self.size[1]
            self.tile = [ImageFile._Tile('raw', (0, 0) + self.size, offs, ('G', 0, -1)), ImageFile._Tile('raw', (0, 0) + self.size, offs + size, ('R', 0, -1)), ImageFile._Tile('raw', (0, 0) + self.size, offs + 2 * size, ('B', 0, -1))]
        else:
