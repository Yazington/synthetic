...
def _write_image(im: Image.Image, filename: str | bytes, existing_pdf: PdfParser.PdfParser, image_refs: list[PdfParser.IndirectReference]) -> tuple[PdfParser.IndirectReference, str]:
    params = None
    decode = None
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes, save_all: bool=False) -> None:
    ...
    for im_sequence in ims:
        im_pages: ImageSequence.Iterator | list[Image.Image] = ImageSequence.Iterator(im_sequence) if save_all else [im_sequence]
        for im in im_pages:
