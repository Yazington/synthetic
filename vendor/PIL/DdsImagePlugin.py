...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    if im.mode[0] == 'L':
        ...
        rawmode = im.mode
        if alpha:
            rgba_mask = [255, 255, 255]
        else:
            rgba_mask = [4278190080, 4278190080, 4278190080]
    else:
        pixel_flags = DDPF.RGB
        rawmode = im.mode[::-1]
        rgba_mask = [16711680, 65280, 255]
        if alpha:
            (r, g, b, a) = im.split()
            im = Image.merge('RGBA', (a, r, g, b))
    if alpha:
        pixel_flags |= DDPF.ALPHAPIXELS
    rgba_mask.append(4278190080 if alpha else 0)
    flags = DDSD.CAPS | DDSD.HEIGHT | DDSD.WIDTH | DDSD.PITCH | DDSD.PIXELFORMAT
    bitcount = len(im.getbands()) * 8
    pitch = (im.width * bitcount + 7) // 8
    fp.write(o32(DDS_MAGIC) + struct.pack('<7I', 124, flags, im.height, im.width, pitch, 0, 0) + struct.pack('11I', *(0,) * 11) + struct.pack('<4I', 32, pixel_flags, 0, bitcount) + struct.pack('<4I', *rgba_mask) + struct.pack('<5I', DDSCAPS.TEXTURE, 0, 0, 0, 0))
