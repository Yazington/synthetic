...
def APP(self: JpegImageFile, marker: int) -> None:
    ...
    if marker == 65504 and s[:4] == b'JFIF':
    ...
    elif marker == 65505 and s[:29] == b'http://ns.adobe.com/xap/1.0/\x00':
        ...
        while s[offset:offset + 4] == b'8BIM':
            try:
                ...
                if code == 1005:
                    photoshop[code] = {'XResolution': i32(data, 0) / 65536, 'DisplayedUnitsX': i16(data, 4), 'YResolution': i32(data, 8) / 65536, 'DisplayedUnitsY': i16(data, 12)}
                else:
                    photoshop[code] = data
                offset += size
                offset += offset & 1
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    def validate_qtables(qtables: str | tuple[list[int], ...] | list[list[int]] | dict[int, list[int]] | None) -> list[list[int]] | None:
        if qtables is None:
            return qtables
    ...
    if len(exif) > MAX_BYTES_IN_MARKER:
        ...
        raise ValueError(msg)
    im.encoderconfig = (quality, progressive, info.get('smooth', 0), optimize, info.get('keep_rgb', False), info.get('streamtype', 0), dpi[0], dpi[1], subsampling, info.get('restart_marker_blocks', 0), info.get('restart_marker_rows', 0), qtables, comment, extra, exif)
    bufsize = 0
