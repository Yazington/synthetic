...
class PdfParser:
    ...
    def write_xref_and_trailer(self, new_root_ref: IndirectReference | None=None) -> None:
        ...
        trailer_dict: dict[str | bytes, Any] = {b'Root': self.root_ref, b'Size': num_entries}
    ...
    def interpret_trailer(cls, trailer_data: bytes) -> dict[bytes, Any]:
        ...
        while True:
            ...
            if not m:
                ...
                check_format_condition(m is not None and m.end() == len(trailer_data), 'name not found in trailer, remaining data: ' + repr(trailer_data[offset:]))
                break
    ...
    @classmethod
    def get_value(cls, data: bytes | bytearray | mmap.mmap, offset: int, expect_indirect: IndirectReference | None=None, max_nesting: int=-1) -> tuple[Any, int | None]:
        if max_nesting == 0:
            return (None, None)
        ...
        if m:
            ...
            while not m:
                ...
                (key, current_offset) = cls.get_value(data, current_offset, max_nesting=max_nesting - 1)
                if current_offset is None:
                    return (result, None)
                (value, current_offset) = cls.get_value(data, current_offset, max_nesting=max_nesting - 1)
                result[key] = value
                if current_offset is None:
                    return (result, None)
