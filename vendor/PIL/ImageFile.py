...
class ImageFile(Image.Image):
    ...
    def _seek_check(self, frame: int) -> bool:
        if frame < self._min_frame or (not (hasattr(self, '_n_frames') and self._n_frames is None) and frame >= getattr(self, 'n_frames') + self._min_frame):
...
def _encode_tile(im: Image.Image, fp: IO[bytes], tile: list[_Tile], bufsize: int, fh: int | None, exc: BaseException | None=None) -> None:
...
class PyCodec:
    ...
    def setimage(self, im: Image.core.ImagingCore, extents: tuple[int, int, int, int] | None=None) -> None:
        """
