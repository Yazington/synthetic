...
def colorize(image: Image.Image, black: str | tuple[int, ...], white: str | tuple[int, ...], mid: str | int | tuple[int, ...] | None=None, blackpoint: int=0, whitepoint: int=255, midpoint: int=127) -> Image.Image:
    """
...
def pad(image: Image.Image, size: tuple[int, int], method: int=Image.Resampling.BICUBIC, color: str | int | tuple[int, ...] | None=None, centering: tuple[float, float]=(0.5, 0.5)) -> Image.Image:
    """
            ...
            for key in ('XML:com.adobe.xmp', 'xmp'):
                if key in exif_image.info:
                    for pattern in ('tiff:Orientation="([0-9])"', '<tiff:Orientation>([0-9])</tiff:Orientation>'):
                        ...
                        exif_image.info[key] = re.sub(pattern, '', value) if isinstance(value, str) else re.sub(pattern.encode(), b'', value)
        if not in_place:
