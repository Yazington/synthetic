...
from ..sentry_sdk.utils import AnnotatedValue, capture_internal_exception, disable_capture_event, format_timestamp, json_dumps, safe_repr, strip_string
__import__('.'.join(__package__.split('.')[:-1] + ['sentry_sdk.utils']), globals(), locals(), [], 0)
...
def add_global_repr_processor(processor): # type: (ReprProcessor) -> None
...
class Memo(object):
    ...
    def memoize(self, obj): # type: (Any) -> ContextManager[bool]
        self._objs.append(obj)
        return self

    def __enter__(self): # type: () -> bool
        obj = self._objs[-1]
    ...
    def __exit__(self, ty, value, tb): # type: (...) -> None
        self._ids.pop(id(self._objs.pop()), None)

def serialize(event, smart_transaction_trimming=False, **kwargs): # type: (Event, bool, **Any) -> Event
    memo = Memo()
    path = [] # type: List[Segment]
    meta_stack = [] # type: List[Dict[str, Any]]
    span_description_bytes = [] # type: List[int]

    def _annotate(**meta): # type: (**Any) -> None
    ...
    def _should_repr_strings(): # type: () -> Optional[bool]
        """
        ...
        try:
            ...
            if p0 == 'stacktrace' and path[1] == 'frames' and (path[3] == 'vars'):
                return True
            if p0 in ('threads', 'exception') and path[1] == 'values' and (path[3] == 'stacktrace') and (path[4] == 'frames') and (path[6] == 'vars'):
                return True
        except IndexError:
            return None
        return False
    ...
    def _serialize_node(obj, is_databag=None, should_repr_strings=None, segment=None, remaining_breadth=None, remaining_depth=None): # type: (...) -> Any
        ...
        try:
            with memo.memoize(obj) as result:
                if result:
                    return CYCLE_MARKER
                return _serialize_node_impl(obj, is_databag=is_databag, should_repr_strings=should_repr_strings, remaining_depth=remaining_depth, remaining_breadth=remaining_breadth)
    ...
    def _flatten_annotated(obj): # type: (Any) -> Any
        if isinstance(obj, AnnotatedValue):
            _annotate(**obj.metadata)
            obj = obj.value
        return obj

    def _serialize_node_impl(obj, is_databag, should_repr_strings, remaining_depth, remaining_breadth): # type: (Any, Optional[bool], Optional[bool], Optional[int], Optional[int]) -> Any
        ...
        if obj is None or isinstance(obj, (bool, number_types)):
            ...
            for (k, v) in iteritems(obj):
                if remaining_breadth is not None and i >= remaining_breadth:
                    ...
                    break
                str_k = text_type(k)
                v = _serialize_node(v, segment=str_k, should_repr_strings=should_repr_strings, is_databag=is_databag, remaining_depth=remaining_depth - 1 if remaining_depth is not None else None, remaining_breadth=remaining_breadth)
                rv_dict[str_k] = v
                i += 1
            return rv_dict
            ...
            for (i, v) in enumerate(obj):
                if remaining_breadth is not None and i >= remaining_breadth:
                    _annotate(len=len(obj))
                    break
                rv_list.append(_serialize_node(v, segment=i, should_repr_strings=should_repr_strings, is_databag=is_databag, remaining_depth=remaining_depth - 1 if remaining_depth is not None else None, remaining_breadth=remaining_breadth))
            return rv_list
        if should_repr_strings:
            ...
            if not isinstance(obj, string_types):
                obj = safe_repr(obj)
        if smart_transaction_trimming and len(path) == 3 and (path[0] == 'spans') and (path[-1] == 'description'):
            span_description_bytes.append(len(obj))
            return obj
        return _flatten_annotated(strip_string(obj))

    def _truncate_span_descriptions(serialized_event, event, excess_bytes): # type: (Event, Event, int) -> None
        """
        ...
        def shortest_duration_longest_description_first(args): # type: (Tuple[int, Span]) -> Tuple[timedelta, int]
        ...
        path.append('spans')
        for (i, span) in sorted(enumerate(serialized_event.get('spans') or []), key=shortest_duration_longest_description_first):
            description = span.get('description') or ''
            if len(description) <= min_length:
                continue
            excess_bytes -= len(description) - min_length
            path.extend([i, 'description'])
            span['description'] = _flatten_annotated(strip_string(description, max_length=min_length))
            del path[-2:]
            del meta_stack[len(path) + 1:]
            if excess_bytes <= 0:
                break
