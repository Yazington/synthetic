...
def is_auto_session_tracking_enabled(hub=None): # type: (Optional[sentry_sdk.Hub]) -> bool
...
@contextmanager
def auto_session_tracking(hub=None, session_mode='application'): # type: (Optional[sentry_sdk.Hub], str) -> Generator[None, None, None]
...
def make_aggregate_envelope(aggregate_states, attrs): # type: (Any, Any) -> Any
    return {'attrs': dict(attrs), 'aggregates': list(aggregate_states.values())}

class SessionFlusher(object):

    def __init__(self, capture_func, flush_interval=60): # type: (...) -> None
    ...
    def _ensure_running(self): # type: (...) -> None
    ...
    def add_aggregate_session(self, session): # type: (...) -> None
        with self._aggregate_lock:
