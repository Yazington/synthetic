...
if getattr(Request, 'authenticated_userid', None):
    ...
    def authenticated_userid(request): # type: (Request) -> Optional[Any]
...
class PyramidIntegration(Integration):
    ...
    def __init__(self, transaction_style='route_name'): # type: (str) -> None
    ...
    def setup_once(): # type: () -> None
        ...
        def sentry_patched_call_view(registry, request, *args, **kwargs): # type: (Any, Request, *Any, **Any) -> Response
            hub = Hub.current
        ...
        if hasattr(Request, 'invoke_exception_view'):
            ...
            def sentry_patched_invoke_exception_view(self, *args, **kwargs): # type: (Request, *Any, **Any) -> Any
        ...
        def sentry_patched_wsgi_call(self, environ, start_response): # type: (Any, Dict[str, str], Callable[..., Any]) -> _ScopedResponse
            hub = Hub.current
            integration = hub.get_integration(PyramidIntegration)
            if integration is None:
                return old_wsgi_call(self, environ, start_response)

            def sentry_patched_inner_wsgi_call(environ, start_response): # type: (Dict[str, Any], Callable[..., Any]) -> Any
                try:
                    return old_wsgi_call(self, environ, start_response)
                except Exception:
                    einfo = sys.exc_info()
...
def _capture_exception(exc_info): # type: (ExcInfo) -> None
    ...
    (event, hint) = event_from_exception(exc_info, client_options=client.options, mechanism={'type': 'pyramid', 'handled': False})
...
def _make_event_processor(weak_request, integration): # type: (Callable[[], Request], PyramidIntegration) -> EventProcessor
