...
def get_template_frame_from_exception(exc_value): # type: (Optional[BaseException]) -> Optional[Dict[str, Any]]
...
def _get_template_name_description(template_name): # type: (str) -> str
...
def patch_templates(): # type: () -> None
    from django.template.response import SimpleTemplateResponse
    from ....sentry_sdk.integrations.django import DjangoIntegration
    real_rendered_content = SimpleTemplateResponse.rendered_content

    @property
    def rendered_content(self): # type: (SimpleTemplateResponse) -> str
        hub = Hub.current
        if hub.get_integration(DjangoIntegration) is None:
            return real_rendered_content.fget(self)
        with hub.start_span(op='django.template.render', description=_get_template_name_description(self.template_name)) as span:
    ...
    def render(request, template_name, context=None, *args, **kwargs): # type: (django.http.HttpRequest, str, Optional[Dict[str, Any]], *Any, **Any) -> django.http.HttpResponse
        hub = Hub.current
        if hub.get_integration(DjangoIntegration) is None:
            return real_render(request, template_name, context, *args, **kwargs)
        with hub.start_span(op='django.template.render', description=_get_template_name_description(template_name)) as span:
            span.set_data('context', context)
            return real_render(request, template_name, context, *args, **kwargs)
    django.shortcuts.render = render

def _get_template_frame_from_debug(debug): # type: (Dict[str, Any]) -> Dict[str, Any]
    if debug is None:
        return None
    lineno = debug['line']
    filename = debug['name']
...
def _linebreak_iter(template_source): # type: (str) -> Iterator[int]
    yield 0
...
def _get_template_frame_from_source(source): # type: (Tuple[Origin, Tuple[int, int]]) -> Optional[Dict[str, Any]]
    if not source:
        return None
