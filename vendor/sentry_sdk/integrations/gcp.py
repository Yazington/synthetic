...
def _wrap_func(func): # type: (F) -> F

    def sentry_func(functionhandler, gcp_event, *args, **kwargs): # type: (Any, Any, *Any, **Any) -> Any
        hub = Hub.current
        ...
        with hub.push_scope() as scope:
            ...
            transaction = Transaction.continue_from_headers(headers, op='serverless.function', name=environ.get('FUNCTION_NAME', ''))
...
def _make_request_event_processor(gcp_event, configured_timeout, initial_time): # type: (Any, Any, Any) -> EventProcessor
    ...
    def event_processor(event, hint): # type: (Event, Hint) -> Optional[Event]
        ...
        extra['google cloud functions'] = {'function_name': environ.get('FUNCTION_NAME'), 'function_entry_point': environ.get('ENTRY_POINT'), 'function_identity': environ.get('FUNCTION_IDENTITY'), 'function_region': environ.get('FUNCTION_REGION'), 'function_project': environ.get('GCP_PROJECT'), 'execution_duration_in_millis': execution_duration_in_millis, 'configured_timeout_in_seconds': configured_timeout}
        extra['google cloud logs'] = {'url': _get_google_cloud_logs_url(final_time)}
...
def _get_google_cloud_logs_url(final_time): # type: (datetime) -> str
    """
