...
from ...sentry_sdk.utils import ContextVar, capture_internal_exceptions, event_from_exception
...
if MYPY:
    ...
    class StartResponse(Protocol):
        ...
        def __call__(self, status, response_headers, exc_info=None): # type: (str, WsgiResponseHeaders, Optional[WsgiExcInfo]) -> WsgiResponseIter
            pass
_wsgi_middleware_applied = ContextVar('sentry_wsgi_middleware_applied')
if PY2:

    def wsgi_decoding_dance(s, charset='utf-8', errors='replace'): # type: (str, str, str) -> str
...
def get_host(environ, use_x_forwarded_for=False): # type: (Dict[str, str], bool) -> str
...
def get_request_url(environ, use_x_forwarded_for=False): # type: (Dict[str, str], bool) -> str
...
class SentryWsgiMiddleware(object):
    ...
    def __init__(self, app, use_x_forwarded_for=False): # type: (Callable[[Dict[str, str], Callable[..., Any]], Any], bool) -> None
        self.app = app
        self.use_x_forwarded_for = use_x_forwarded_for

    def __call__(self, environ, start_response): # type: (Dict[str, str], Callable[..., Any]) -> _ScopedResponse
        ...
        try:
            ...
            with auto_session_tracking(hub, session_mode='request'):
                with hub:
                    ...
                    transaction = Transaction.continue_from_environ(environ, op='http.server', name='generic WSGI request')
...
def _sentry_start_response(old_start_response, transaction, status, response_headers, exc_info=None): # type: (...) -> WsgiResponseIter
    with capture_internal_exceptions():
...
def _get_environ(environ): # type: (Dict[str, str]) -> Iterator[Tuple[str, str]]
    """
...
def _get_headers(environ): # type: (Dict[str, str]) -> Iterator[Tuple[str, str]]
    """
...
def _capture_exception(hub): # type: (Hub) -> ExcInfo
    exc_info = sys.exc_info()
    if hub.client is not None:
        e = exc_info[1]
        should_skip_capture = isinstance(e, SystemExit) and e.code in (0, None)
        if not should_skip_capture:
            (event, hint) = event_from_exception(exc_info, client_options=hub.client.options, mechanism={'type': 'wsgi', 'handled': False})
...
class _ScopedResponse(object):
    ...
    def __init__(self, hub, response): # type: (Hub, Iterator[bytes]) -> None
        self._hub = hub
...
def _make_wsgi_event_processor(environ, use_x_forwarded_for): # type: (Dict[str, str], bool) -> EventProcessor
