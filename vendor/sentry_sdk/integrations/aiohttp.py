...
from ...sentry_sdk.integrations._wsgi_common import _filter_headers, request_body_within_bounds
from ...sentry_sdk.tracing import Transaction
from ...sentry_sdk.utils import capture_internal_exceptions, event_from_exception, transaction_from_function, HAS_REAL_CONTEXTVARS, CONTEXTVARS_ERROR_MESSAGE, AnnotatedValue
try:
    import asyncio
...
class AioHttpIntegration(Integration):
    ...
    def __init__(self, transaction_style='handler_name'): # type: (str) -> None
    ...
    def setup_once(): # type: () -> None
        ...
        async def sentry_app_handle(self, request, *args, **kwargs): # type: (Any, Request, *Any, **Any) -> Any
            hub = Hub.current
            ...
            with Hub(hub) as hub:
                ...
                transaction = Transaction.continue_from_headers(request.headers, op='http.server', name='generic AIOHTTP request')
        ...
        async def sentry_urldispatcher_resolve(self, request): # type: (UrlDispatcher, Request) -> AbstractMatchInfo
...
def _make_request_processor(weak_request): # type: (Callable[[], Request]) -> EventProcessor
    ...
    def aiohttp_processor(event, hint): # type: (...) -> Dict[str, Any]
...
def _capture_exception(hub): # type: (Hub) -> ExcInfo
    ...
    (event, hint) = event_from_exception(exc_info, client_options=hub.client.options, mechanism={'type': 'aiohttp', 'handled': False})
    hub.capture_event(event, hint=hint)
    return exc_info
BODY_NOT_READ_MESSAGE = "[Can't show request body due to implementation details.]"

def get_aiohttp_request_data(hub, request): # type: (Hub, Request) -> Union[Optional[str], AnnotatedValue]
    bytes_body = request._read_bytes
    if bytes_body is not None:
        if not request_body_within_bounds(hub.client, len(bytes_body)):
            return AnnotatedValue('', {'rem': [['!config', 'x', 0, len(bytes_body)]], 'len': len(bytes_body)})
