...
class RqIntegration(Integration):
    ...
    def setup_once(): # type: () -> None
        ...
        def sentry_patched_perform_job(self, job, *args, **kwargs): # type: (Any, Job, *Queue, **Any) -> bool
            hub = Hub.current
            ...
            with hub.push_scope() as scope:
                ...
                transaction = Transaction.continue_from_headers(job.meta.get('_sentry_trace_headers') or {}, op='rq.task', name='unknown RQ task')
        ...
        def sentry_patched_handle_exception(self, job, *exc_info, **kwargs): # type: (Worker, Any, *Any, **Any) -> Any
            if job.is_failed:
        ...
        def sentry_patched_enqueue_job(self, job, **kwargs): # type: (Queue, Any, **Any) -> Any
            hub = Hub.current
...
def _make_event_processor(weak_job): # type: (Callable[[], Job]) -> EventProcessor
    ...
    def event_processor(event, hint): # type: (Dict[str, Any], Dict[str, Any]) -> Dict[str, Any]
        ...
        if job is not None:
            with capture_internal_exceptions():
                ...
                extra['rq-job'] = {'job_id': job.id, 'func': job.func_name, 'args': job.args, 'kwargs': job.kwargs, 'description': job.description}
        if 'exc_info' in hint:
            with capture_internal_exceptions():
...
def _capture_exception(exc_info, **kwargs): # type: (ExcInfo, **Any) -> None
    hub = Hub.current
    if hub.get_integration(RqIntegration) is None:
        return
    client = hub.client # type: Any
    (event, hint) = event_from_exception(exc_info, client_options=client.options, mechanism={'type': 'rq', 'handled': False})
