...
class EventSourceHandler(ChaliceEventSourceHandler):
    ...
    def __call__(self, event, context): # type: (Any, Any) -> Any
        hub = Hub.current
        client = hub.client # type: Any
        with hub.push_scope() as scope:
            with capture_internal_exceptions():
                configured_time = context.get_remaining_time_in_millis()
                scope.add_event_processor(_make_request_event_processor(event, context, configured_time))
            try:
...
def _get_view_function_response(app, view_function, function_args): # type: (Any, F, Any) -> F

    @wraps(view_function)
    def wrapped_view_function(**function_args): # type: (**Any) -> Any
        hub = Hub.current
        client = hub.client # type: Any
        with hub.push_scope() as scope:
            with capture_internal_exceptions():
...
class ChaliceIntegration(Integration):
    ...
    def setup_once(): # type: () -> None
        ...
        def sentry_event_response(app, view_function, function_args): # type: (Any, F, Dict[str, Any]) -> Any
