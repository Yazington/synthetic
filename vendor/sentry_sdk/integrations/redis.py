...
_SINGLE_KEY_COMMANDS = frozenset(['decr', 'decrby', 'get', 'incr', 'incrby', 'pttl', 'set', 'setex', 'setnx', 'ttl'])
_MULTI_KEY_COMMANDS = frozenset(['del', 'touch', 'unlink'])

def _patch_rediscluster(): # type: () -> None
    try:
        import rediscluster
    except ImportError:
        return
...
def patch_redis_client(cls): # type: (Any) -> None
    """
    ...
    def sentry_patched_execute_command(self, name, *args, **kwargs): # type: (Any, str, *Any, **Any) -> Any
        hub = Hub.current
