...
from ...sentry_sdk.utils import capture_internal_exceptions, event_from_exception, HAS_REAL_CONTEXTVARS, CONTEXTVARS_ERROR_MESSAGE
...
async def _hub_enter(request): # type: (Request) -> None
    hub = Hub.current
...
async def _hub_exit(request, **_): # type: (Request, **Any) -> None
    request.ctx._sentry_hub.__exit__(None, None, None)

async def _set_transaction(request, route, **kwargs): # type: (Request, Route, **Any) -> None
    hub = Hub.current
...
def _sentry_error_handler_lookup(self, exception, *args, **kwargs): # type: (Any, Exception, *Any, **Any) -> Optional[object]
    ...
    async def sentry_wrapped_error_handler(request, exception): # type: (Request, Exception) -> Any
        try:
...
async def _legacy_handle_request(self, request, *args, **kwargs): # type: (Any, Request, *Any, **Any) -> Any
    hub = Hub.current
...
def _legacy_router_get(self, *args): # type: (Any, Union[Any, Request]) -> Any
...
def _capture_exception(exception): # type: (Union[Tuple[Optional[type], Optional[BaseException], Any], BaseException]) -> None
    ...
    with capture_internal_exceptions():
        (event, hint) = event_from_exception(exception, client_options=client.options, mechanism={'type': 'sanic', 'handled': False})
        hub.capture_event(event, hint=hint)

def _make_request_processor(weak_request): # type: (Callable[[], Request]) -> EventProcessor

    def sanic_processor(event, hint): # type: (Event, Optional[Hint]) -> Optional[Event]
        try:
