...
_RUNTIME_CONTEXT = {'name': platform.python_implementation(), 'version': '%s.%s.%s' % sys.version_info[:3], 'build': sys.version}
...
class StdlibIntegration(Integration):
    ...
    def setup_once(): # type: () -> None
        ...
        def add_python_runtime_context(event, hint): # type: (Event, Hint) -> Optional[Event]
...
def _install_httplib(): # type: () -> None
    real_putrequest = HTTPConnection.putrequest
    real_getresponse = HTTPConnection.getresponse

    def putrequest(self, method, url, *args, **kwargs): # type: (HTTPConnection, str, str, *Any, **Any) -> Any
        hub = Hub.current
    ...
    def getresponse(self, *args, **kwargs): # type: (HTTPConnection, *Any, **Any) -> Any
...
def _init_argument(args, kwargs, name, position, setdefault_callback=None): # type: (List[Any], Dict[Any, Any], str, int, Optional[Callable[[Any], Any]]) -> Any
    """
...
def _install_subprocess(): # type: () -> None
    old_popen_init = subprocess.Popen.__init__

    def sentry_patched_popen_init(self, *a, **kw): # type: (subprocess.Popen[Any], *Any, **Any) -> None
        hub = Hub.current
        if hub.get_integration(StdlibIntegration) is None:
    ...
    def sentry_patched_popen_wait(self, *a, **kw): # type: (subprocess.Popen[Any], *Any, **Any) -> Any
        hub = Hub.current
        if hub.get_integration(StdlibIntegration) is None:
    ...
    def sentry_patched_popen_communicate(self, *a, **kw): # type: (subprocess.Popen[Any], *Any, **Any) -> Any
        hub = Hub.current
        if hub.get_integration(StdlibIntegration) is None:
...
def get_subprocess_traceparent_headers(): # type: () -> EnvironHeaders
