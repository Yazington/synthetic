...
class ExcepthookIntegration(Integration):
    ...
    def __init__(self, always_run=False): # type: (bool) -> None
...
def _make_excepthook(old_excepthook): # type: (Excepthook) -> Excepthook

    def sentry_sdk_excepthook(type_, value, traceback): # type: (Type[BaseException], BaseException, TracebackType) -> None
        hub = Hub.current
...
def _should_send(always_run=False): # type: (bool) -> bool
    if always_run:
        return True
    if hasattr(sys, 'ps1'):
        return False
