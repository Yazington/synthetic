...
class _HubBasedClientFilter(logging.Filter):
    ...
    def filter(self, record): # type: (LogRecord) -> bool
...
def init_debug_support(): # type: () -> None
    if not logger.handlers:
        configure_logger()
    configure_debug_hub()

def configure_logger(): # type: () -> None
...
def configure_debug_hub(): # type: () -> None

    def _get_debug_hub(): # type: () -> Hub
        return Hub.current
