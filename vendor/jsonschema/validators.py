...
import typing
import warnings
__import__('.'.join(__package__.split('.')[:-1] + ['attr']), globals(), locals(), [], 0)
from .. import attr
from ..jsonschema import _format, _legacy_validators, _types, _utils, _validators, exceptions
...
def __getattr__(name):
    if name == 'ErrorTree':
        warnings.warn('Importing ErrorTree from jsonschema.validators is deprecated. Instead import it from jsonschema.exceptions.', DeprecationWarning, stacklevel=2)
        from ..jsonschema.exceptions import ErrorTree
        return ErrorTree
    elif name == 'validators':
        warnings.warn('Accessing jsonschema.validators.validators is deprecated. Use jsonschema.validators.validator_for with a given schema.', DeprecationWarning, stacklevel=2)
        return _VALIDATORS
    elif name == 'meta_schemas':
        warnings.warn('Accessing jsonschema.validators.meta_schemas is deprecated. Use jsonschema.validators.validator_for with a given schema.', DeprecationWarning, stacklevel=2)
...
def _store_schema_list():
    ...
    return [(id, validator.META_SCHEMA) for (id, validator) in _META_SCHEMAS.items()] + _VOCABULARIES

def create(meta_schema, validators=(), version=None, type_checker=_types.draft202012_type_checker, format_checker=_format.draft202012_format_checker, id_of=_id_of, applicable_validators=lambda schema: schema.items()):
    """
    ...
    class Validator:
        ...
        def iter_errors(self, instance, _schema=None):
            if _schema is not None:
                warnings.warn('Passing a schema to Validator.iter_errors is deprecated and will be removed in a future release. Call validator.evolve(schema=new_schema).iter_errors(...) instead.', DeprecationWarning, stacklevel=2)
            else:
                _schema = self.schema
            if _schema is True:
                return
            elif _schema is False:
                yield exceptions.ValidationError(f'False schema does not allow {instance!r}', validator=None, validator_value=None, instance=instance, schema=_schema)
                return
            scope = id_of(_schema)
            if scope:
            ...
            try:
                for (k, v) in applicable_validators(_schema):
                    ...
                    for error in errors:
                        error._set(validator=k, validator_value=v, instance=instance, schema=_schema)
                        if k not in {'if', '$ref'}:
        ...
        def is_valid(self, instance, _schema=None):
            if _schema is not None:
                warnings.warn('Passing a schema to Validator.is_valid is deprecated and will be removed in a future release. Call validator.evolve(schema=new_schema).is_valid(...) instead.', DeprecationWarning, stacklevel=2)
...
class RefResolver(object):
    ...
    def from_schema(cls, schema, id_of=_id_of, *args, **kwargs):
        ...
        Returns:
            ...
            `RefResolver`
        """
        return cls(*args, base_uri=id_of(schema), referrer=schema, **kwargs)
    ...
    def pop_scope(self):
        ...
        try:
            ...
            raise exceptions.RefResolutionError('Failed to pop the scope from an empty stack. `pop_scope()` should only be called once for every `push_scope()`')
    ...
    def in_scope(self, scope):
        ...
        """
        warnings.warn('jsonschema.RefResolver.in_scope is deprecated and will be removed in a future release.', DeprecationWarning, stacklevel=3)
    ...
    def _get_subschemas_cache(self):
        ...
        for (keyword, subschema) in _search_schema(self.referrer, _match_subschema_keywords):
    ...
    def resolve_fragment(self, document, fragment):
        ...
        for part in parts:
            ...
            try:
                ...
                raise exceptions.RefResolutionError(f'Unresolvable JSON pointer: {fragment!r}')
        return document
