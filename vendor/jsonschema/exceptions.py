...
class _Error(Exception):
    ...
    def __init__(self, message, validator=_unset, path=(), cause=None, context=(), validator_value=_unset, instance=_unset, schema=_unset, schema_path=(), parent=None):
        super(_Error, self).__init__(message, validator, path, cause, context, validator_value, instance, schema, schema_path, parent)
    ...
    def __str__(self):
        essential_for_verbose = (self.validator, self.validator_value, self.instance, self.schema)
        if any((m is _unset for m in essential_for_verbose)):
            return self.message
        schema_path = _utils.format_as_index(container=self._word_for_schema_in_error_message, indices=list(self.relative_schema_path)[:-1])
        instance_path = _utils.format_as_index(container=self._word_for_instance_in_error_message, indices=self.relative_path)
        prefix = 16 * ' '
...
class UnknownType(Exception):
    ...
    def __str__(self):
        prefix = 16 * ' '
        return dedent(f'            Unknown type {self.type!r} for validator with schema:\n                {indent(pformat(self.schema, width=72), prefix).lstrip()}\n\n            While checking instance:\n                {indent(pformat(self.instance, width=72), prefix).lstrip()}\n            '.rstrip())
