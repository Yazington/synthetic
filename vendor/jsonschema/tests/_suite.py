...
class Suite(object):
    ...
    def _remotes(self):
        ...
        remotes = subprocess.check_output([sys.executable, str(jsonschema_suite), 'remotes'])
        return {'http://localhost:1234/' + name.replace('\\', '/'): schema for (name, schema) in json.loads(remotes.decode('utf-8')).items()}

    def benchmark(self, runner):
        for (name, Validator) in _VALIDATORS.items():
            self.version(name=name).benchmark(runner=runner, Validator=Validator)

    def version(self, name):
        return Version(name=name, path=self._root.joinpath('tests', name), remotes=self._remotes())
...
class Version(object):
    ...
    def tests(self):
        return (test for child in self._path.glob('*.json') for test in self._tests_in(subject=child.name[:-5], path=child))

    def format_tests(self):
        path = self._path.joinpath('optional', 'format')
        return (test for child in path.glob('*.json') for test in self._tests_in(subject=child.name[:-5], path=child))

    def optional_tests_of(self, name):
        return self._tests_in(subject=name, path=self._path.joinpath('optional', name + '.json'))
...
class _Test(object):
    ...
    def method_name(self):
        delimiters = '[\\W\\- ]+'
        return 'test_{}_{}_{}'.format(re.sub(delimiters, '_', self.subject), re.sub(delimiters, '_', self.case_description), re.sub(delimiters, '_', self.description))
