...
"""
import sys
from ...jsonschema import Draft3Validator, Draft4Validator, Draft6Validator, Draft7Validator, Draft201909Validator, Draft202012Validator, draft3_format_checker, draft4_format_checker, draft6_format_checker, draft7_format_checker, draft201909_format_checker, draft202012_format_checker
...
def missing_format(checker):
    ...
    def missing_format(test):
        schema = test.schema
        if schema is True or schema is False or 'format' not in schema or (schema['format'] in checker.checkers) or test.valid:
            return
...
if sys.version_info < (3, 9):
    ...
    allowed_leading_zeros = skip(message=message, subject='ipv4', description='leading zeroes should be rejected, as they are treated as octals')
else:
...
TestDraft202012Format = DRAFT202012.to_unittest_testcase(DRAFT202012.format_tests(), Validator=Draft202012Validator, format_checker=draft202012_format_checker, skip=lambda test: complex_email_validation(test) or allowed_leading_zeros(test) or leap_second(test) or missing_format(draft202012_format_checker)(test) or complex_email_validation(test))
