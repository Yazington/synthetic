...
import re
from ..jsonschema._utils import ensure_list, equal, extras_msg, find_additional_properties, find_evaluated_item_indexes_by_schema, find_evaluated_property_keys_by_schema, unbool, uniq
    ...
    for (pattern, subschema) in patternProperties.items():
        for (k, v) in instance.items():
            if re.search(pattern, k):
                yield from validator.descend(v, subschema, path=k, schema_path=pattern)
...
def items(validator, items, instance, schema):
    ...
    if items is False and total > prefix:
        ...
        for index in range(prefix, total):
            yield from validator.descend(instance=instance[index], schema=items, path=index)
    ...
    for each in instance:
        if validator.evolve(schema=contains).is_valid(each):
            ...
            if matches > max_contains:
                yield ValidationError(f'Too many items match the given schema (expected at most {max_contains})', validator='maxContains', validator_value=max_contains)
                return
    if matches < min_contains:
        if not matches:
            yield ValidationError(f'{instance!r} does not contain items matching the given schema')
        else:
            yield ValidationError(f'Too few items match the given schema (expected at least {min_contains} but only {matches} matched)', validator='minContains', validator_value=min_contains)
...
def dynamicRef(validator, dynamicRef, instance, schema):
    ...
    for url in validator.resolver._scopes_stack:
        ...
        with validator.resolver.resolving(lookup_url) as subschema:
            if '$dynamicAnchor' in subschema and fragment == subschema['$dynamicAnchor']:
