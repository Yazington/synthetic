...
def find_evaluated_item_indexes_by_schema(validator, instance, schema):
    ...
    if '$ref' in schema:
        ...
        try:
            evaluated_indexes += find_evaluated_item_indexes_by_schema(validator, instance, resolved)
        finally:
...
def find_evaluated_property_keys_by_schema(validator, instance, schema):
    ...
    if '$ref' in schema:
        ...
        validator.resolver.push_scope(scope)
        try:
            evaluated_keys += find_evaluated_property_keys_by_schema(validator, instance, resolved)
        finally:
            validator.resolver.pop_scope()
        ...
        if keyword in schema:
            if validator.is_type(schema[keyword], 'boolean'):
                for (property, value) in instance.items():
                    if validator.evolve(schema=schema[keyword]).is_valid({property: value}):
                        evaluated_keys.append(property)
            if validator.is_type(schema[keyword], 'object'):
                for (property, subschema) in schema[keyword].items():
                    if property in instance and validator.evolve(schema=subschema).is_valid(instance[property]):
                        evaluated_keys.append(property)
    if 'patternProperties' in schema:
        for (property, value) in instance.items():
            for (pattern, _) in schema['patternProperties'].items():
                if re.search(pattern, property) and validator.evolve(schema=schema['patternProperties']).is_valid({property: value}):
                    evaluated_keys.append(property)
    if 'dependentSchemas' in schema:
        for (property, subschema) in schema['dependentSchemas'].items():
            if property not in instance:
                continue
            evaluated_keys += find_evaluated_property_keys_by_schema(validator, instance, subschema)
    for keyword in ['allOf', 'oneOf', 'anyOf']:
        if keyword in schema:
            for subschema in schema[keyword]:
                errs = list(validator.descend(instance, subschema))
    ...
    if 'if' in schema:
        if validator.evolve(schema=schema['if']).is_valid(instance):
            evaluated_keys += find_evaluated_property_keys_by_schema(validator, instance, schema['if'])
            if 'then' in schema:
                evaluated_keys += find_evaluated_property_keys_by_schema(validator, instance, schema['then'])
        elif 'else' in schema:
            evaluated_keys += find_evaluated_property_keys_by_schema(validator, instance, schema['else'])
