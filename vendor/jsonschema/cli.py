...
class _PrettyFormatter(object):
    ...
    def filenotfound_error(self, path, exc_info):
        return self._ERROR_MSG.format(path=path, type='FileNotFoundError', body='{!r} does not exist.'.format(path))
...
parser.add_argument('-i', '--instance', action='append', dest='instances', help='\n        a path to a JSON instance (i.e. filename.json) to validate (may\n        be specified multiple times). If no instances are provided via this\n        option, one will be expected on standard input.\n    ')
parser.add_argument('-F', '--error-format', help="\n        the format to use for each validation error message, specified\n        in a form suitable for str.format. This string will be passed\n        one formatted object named 'error' for each ValidationError.\n        Only provide this option when using --output=plain, which is the\n        default. If this argument is unprovided and --output=plain is\n        used, a simple default representation will be used.\n    ")
parser.add_argument('-o', '--output', choices=['plain', 'pretty'], default='plain', help="\n        an output format to use. 'plain' (default) will produce minimal\n        text with one line for each error, while 'pretty' will produce\n        more detailed human-readable output on multiple lines.\n    ")
parser.add_argument('-V', '--validator', type=_namedAnyWithDefault, help='\n        the fully qualified object name of a validator to use, or, for\n        validators that are registered with jsonschema, simply the name\n        of the class.\n    ')
parser.add_argument('--base-uri', help='\n        a base URI to assign to the provided schema, even if it does not\n        declare one (via e.g. $id). This option can be used if you wish to\n        resolve relative references to a particular URI (or local path)\n    ')
parser.add_argument('--version', action='version', version=metadata.version('jsonschema'))
parser.add_argument('schema', help='the path to a JSON Schema to validate with (i.e. schema.json)')
...
def run(arguments, stdout=sys.stdout, stderr=sys.stderr, stdin=sys.stdin):
    outputter = _Outputter.from_arguments(arguments=arguments, stdout=stdout, stderr=stderr)
    try:
    ...
    try:
        ...
        outputter.validation_error(instance_path=arguments['schema'], error=error)
        return 1
    if arguments['instances']:
        ...
        def load(_):
            try:
                ...
                outputter.parsing_error(path='<stdin>', exc_info=sys.exc_info())
                raise _CannotLoadFile()
        instances = ['<stdin>']
    resolver = RefResolver(base_uri=arguments['base_uri'], referrer=schema) if arguments['base_uri'] is not None else None
    ...
    for each in instances:
        try:
            ...
            exit_code |= _validate_instance(instance_path=each, instance=instance, validator=validator, outputter=outputter)
    return exit_code
