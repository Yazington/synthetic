...
def load_file(filepath: str, max_size: tuple) -> dict:
    ...
    with open(filepath, 'rb') as bip:
        ...
        if magic.startswith(BIP_FORMATS['BIP2'].magic):
            ...
            return {'icon_size': icon_size, 'icon_pixels': icon_pixels, 'image_size': image_size, 'image_pixels': image_pixels}
    if support_pillow():
        with Image.open(filepath) as image:
            ...
            assert len(image_pixels) == length, 'unexpected amount of pixels'
            data = {'icon_size': image.size, 'icon_pixels': image_pixels, 'image_size': image.size, 'image_pixels': image_pixels}
...
def _resize_image(image: 'Image.Image', max_size: tuple) -> 'Image.Image':
    ...
    scale = min(max_size[0] / image.size[0] if max_size[0] else 1, max_size[1] / image.size[1] if max_size[1] else 1)
