...
def _make_cached_property_getattr(cached_properties, original_getattr, cls):
    lines = ['def wrapper(_cls):', '    __class__ = _cls', '    def __getattr__(self, item, cached_properties=cached_properties, original_getattr=original_getattr, _cached_setattr_get=_cached_setattr_get):', '         func = cached_properties.get(item)', '         if func is not None:', '              result = func(self)', '              _setter = _cached_setattr_get(self)', '              _setter(item, result)', '              return result']
    if original_getattr is not None:
        lines.append('         return original_getattr(self, item)')
    else:
        lines.extend(['         try:', '             return super().__getattribute__(item)', '         except AttributeError:', "             if not hasattr(super(), '__getattr__'):", '                 raise', '             return super().__getattr__(item)', '         original_error = f"\'{self.__class__.__name__}\' object has no attribute \'{item}\'"', '         raise AttributeError(original_error)'])
    lines.extend(['    return __getattr__', '__getattr__ = wrapper(_cls)'])
    unique_filename = _generate_unique_filename(cls, 'getattr')
    glob = {'cached_properties': cached_properties, '_cached_setattr_get': _OBJ_SETATTR.__get__, 'original_getattr': original_getattr}
...
def attrs(maybe_cls=None, these=None, repr_ns=None, repr=None, cmp=None, hash=None, init=None, slots=False, frozen=False, weakref_slot=True, str=False, auto_attribs=False, kw_only=False, cache_hash=False, auto_exc=False, eq=None, order=None, auto_detect=False, collect_by_mro=False, getstate_setstate=None, on_setattr=None, field_transformer=None, match_args=True, unsafe_hash=None):
    ...
    *never* go away, though).
    ...
    if repr_ns is not None:
        import warnings
        warnings.warn(DeprecationWarning('The `repr_ns` argument is deprecated and will be removed in or after August 2025.'), stacklevel=2)
...
def _make_init(cls, attrs, pre_init, pre_init_has_args, post_init, frozen, slots, cache_hash, base_attr_map, is_exc, cls_on_setattr, attrs_init):
    ...
    (script, globs, annotations) = _attrs_to_init_script(filtered_attrs, frozen, slots, pre_init, pre_init_has_args, post_init, cache_hash, base_attr_map, is_exc, needs_cached_setattr, has_cls_on_setattr, '__attrs_init__' if attrs_init else '__init__')
...
def _determine_setters(frozen: bool, slots: bool, base_attr_map: dict[str, type]):
    ...
    if frozen is True:
        ...
        def fmt_setter_with_converter(attr_name: str, value_var: str, has_on_setattr: bool, converter: Converter) -> str:
        ...
        return (('_inst_dict = self.__dict__',), fmt_setter, fmt_setter_with_converter)
        ...
        if a.init is False:
            if has_factory:
                ...
                if converter is not None:
                    lines.append(fmt_setter_with_converter(attr_name, init_factory_name + f'({maybe_self})', has_on_setattr, converter))
                ...
                lines.append(fmt_setter_with_converter(attr_name, f"attr_dict['{attr_name}'].default", has_on_setattr, converter))
                names_for_globals[converter._get_global_name(a.name)] = converter.converter
            else:
            ...
            if converter is not None:
                lines.append('    ' + fmt_setter_with_converter(attr_name, arg_name, has_on_setattr, converter))
                lines.append('else:')
                lines.append('    ' + fmt_setter_with_converter(attr_name, init_factory_name + '(' + maybe_self + ')', has_on_setattr, converter))
                names_for_globals[converter._get_global_name(a.name)] = converter.converter
            else:
                lines.append('    ' + fmt_setter(attr_name, arg_name, has_on_setattr))
                lines.append('else:')
